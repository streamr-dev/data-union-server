...
import { MonoplasmaMember } from './member';
 ...
 */
class MonoplasmaState {
    private id : Number;
    private currentTimestamp : Number;
    private store : Store;
    private blockFreezeSeconds : Number;
    private totalEarnings : BigNumber;
    private log;
    private adminMember : MonoplasmaMember;
    private members : Map<String, MonoplasmaMember>;
    private adminAddress: String;
    private treeCache;
    private treeCacheSize;
    private currentBlock : Number;
    //private indexOf : Map<String, Number>;
    private adminFeeFraction : BigNumber;
    /**
     ...
     */
    constructor(
        blockFreezeSeconds : Number,
        initialMembers,
        store : Store,
        adminAddress : String,
        adminFeeFraction,
        initialBlockNumber : Number = 0,
        ...
        this.totalEarnings = initialMembers.reduce((sum, m) => sum.add(m.earnings), new BigNumber(0))
        ...
        this.members = new Map<String, MonoplasmaMember>()
        initialMembers.forEach((m, i) => { this.members.set(m.address, new MonoplasmaMember(m.name, m.address, m.earnings, m.active)) })

        /** @property {string}  adminAddress the owner address who receives the admin fee and the default payee if no memebers */
        this.adminAddress = adminAddress
        /** @property {BigNumber}  adminFeeFraction fraction of revenue that goes to admin */
        ...
        this.adminMember = this.members.get(adminAddress)
    ...
    getMemberCount() {
        ...
        const total = this.members.size - (this.adminMember.isActive() ? 0 : 1)
        const active = Array.from(this.members.values()).filter(m => m.isActive()).length
        return {
            total,
            active,
    ...
    /**
     * @param {Number|String|BigNumber} adminFeeFraction fraction of revenue that goes to admin (string should be scaled by 10**18, like ether)
     */
    setAdminFeeFraction(adminFeeFraction) {
        ...
        if (typeof adminFeeFraction === "number") {
            ...
            adminFeeFraction = new BigNumber(adminFeeFraction)
        } else if (!adminFeeFraction || adminFeeFraction.constructor !== BigNumber) {
            throw new Error("setAdminFeeFraction: expecting a number, a string, or a BigNumber.js bignumber, got " + JSON.stringify(adminFeeFraction))
        }
